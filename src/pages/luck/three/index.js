...
class App extends React.Component {
	...
	componentDidMount () {
		...
		if (this.nowWinData.list[2].phones.length < this.nowWinData.list[2].num) {
			this.over = false
		} else {
			this.over = true
		}
		...
		if (e.keyCode === 32) {
			if (!!this.over) {
				...
				this.childs.map((item) => {
					...
					this.nowWinData.list[2].phones.push(phone)
	...
	onRef (ref) {
		if (!!this.over) {
			ref.stop(this.nowWinData.list[2].phones[this.childs.length] + '')
		}
		this.childs.push(ref)
	}
	nextClick () {
		if (!this.over) {
			message.error('请先抽取三等奖')
			return false
		}
	...
	render () {
		if (this.nowWinData.list == null) {
			...
			var items = [];
			for (var i = 0; i < this.nowWinData.list[2].num; i++) {
				items.push(<NumBox onRef={this.onRef.bind(this)} key={i}></NumBox>);
			}
			return (
				<div id="luck-three">
					<img className="win_title" src="https://static2.kofuf.com/1573717459686.png" alt="" />
					<div className="num_box_warp">
						{items}
						<div className="num_box_two"></div>
					</div>
